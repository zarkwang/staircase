...
def get_best_result(model, bounds, n_class, n_init_point, 
    ...
    if name_suffix:
        file_name += f"_{name_suffix}"
    file_name = f"models_psm/{file_name}.pkl"
...
def bisection_search(model, bounds, n_class, n_init_point, 
    ...
    os.remove(f'models_psm/{name_prefix}_class_{n_class}_1.pkl')
    os.remove(f'models_psm/{name_prefix}_class_{n_class}_2.pkl')
