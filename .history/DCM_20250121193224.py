...
class mixedNormal(choiceModel):
    ...
    def updateMetaParam(self, multiplier = 5):
        ...
        new_like = self.likeIndiv(const)

        self.obj_func = (multiplier * np.sum(self.w) - np.sum(self.w * np.log(self.likeIndiv(const) * np.exp(multiplier)))) / self.n_random
