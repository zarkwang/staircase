...
class mixedDiscrete(choiceModel):
    ...
    def predict(self):
        ...
        if hasattr(self, 'latent_class') :
            new_like = new_like * self.post_prob[:,np.newaxis]
            p = new_like * self.choice[:,np.newaxis] + (1 - new_like) * (1-self.choice)[:,np.newaxis]

            self.pred = (new_like * self.post_prob[:,np.newaxis]).sum(axis=2) > 0.5

            accuracy = np.sum((self.pred - self.choice) == 0) / self.choice.size
             

            pred_result = {'accuracy': accuracy,'cross-entropy': }
        else:
