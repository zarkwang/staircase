...
class choiceModel:
    ...
    def predict_const(self):
        ...
        if self.sample_weight is not None:
            ...
            w = np.tile(1/new_like.size, new_like.shape)

        accuracy = np.sum((self.pred - self.choice) == 0) / self.pred.size
        loglike = (np.log(new_like) * self.choice  + np.log(1 - new_like) * (1-self.choice)).sum()             

        pred_result = {'accuracy': accuracy,'cross-entropy': - loglike / new_like.size}
...
class mixedDiscrete(choiceModel):
    ...
    def predict(self):
        ...
        if hasattr(self, 'latent_class') :
            ...
            y = np.expand_dims(self.choice, axis=-1)

            if self.sample_weight is not None:
                w = (self.sample_weight / sum(self.sample_weight))[:,np.newaxis]
            else:
                w = np.tile(1/new_like.size, self.pred.shape)

            self.pred = new_like.sum(axis=2) > 0.5

            accuracy = np.sum(np.sum((self.pred - self.choice) == 0) * w)
            cross_ent = np.sum((np.log(new_like) * y  + np.log(1 - new_like) * (1-y)) * w)
             
            pred_result = {'accuracy': accuracy,'cross-entropy': - cross_ent}
        else:
