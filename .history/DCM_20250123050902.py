...
class choiceModel:
    ...
    def predict_const(self):
        ...
        if self.sample_weight is not None:
            w = (self.sample_weight / sum(self.sample_weight))[:,np.newaxis] / (self.sample_weight / sum(self.sample_weight))
        else:
            w = np.tile(1/new_like.size, new_like.shape)

        accuracy = np.sum(((self.pred - self.choice) == 0) * w)
        cross_ent = np.sum((np.log(new_like) * self.choice  + np.log(1 - new_like) * (1-self.choice)) * w / n_question)          
...
class mixedDiscrete(choiceModel):
    ...
    def predict(self):
        ...
        if hasattr(self, 'latent_class') :
            ...
            accuracy = np.sum(((self.pred - self.choice) == 0) * w / n_question)
            cross_ent = np.sum((np.log(new_like) * y  + np.log(1 - new_like) * (1-y)).sum(axis=2) * w / n_question)          
